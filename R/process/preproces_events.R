...
data <- read.csv("Data/data_Emotional_events_in_daily_life.csv")
names(data)
...
data <- data |> mutate(
  ...
  mutate(
    ...
    neg_aff = mean(c(anger, sad, stress), na.rm = TRUE)
  ) |>
  ungroup() |>
...
saveRDS(data_pp, "Data/data_pp_events.RDS")
saveRDS(data, "Data/data_events.RDS")
