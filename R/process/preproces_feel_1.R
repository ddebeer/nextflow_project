...
data <- read.csv("Data/data_FEEL_Study_1.csv")
...
data <- data |> mutate(
  ...
  mutate(
    ...
    neg_aff = mean(c(anger, stress, sad), na.rm = TRUE)
  ) |>
  ungroup() |>
  select(age, gender, erq, er, er_b,
         anger, sad, neg_aff, # stress,
         UUID,
...
saveRDS(data_pp, "Data/data_pp_feel1.RDS")
saveRDS(data, "Data/data_feel1.RDS")
