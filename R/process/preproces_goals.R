...
data <- read.csv("Data/data_Goals_Emotions_Moods_and_Strategies_GEMS_2024.csv")
names(data)
...
data <- data |> mutate(
  ...
  rowwise() |>
  mutate(
    erq = (mean(c(RESS_3_BL, RESS_11_BL, RESS_14_BL, RESS_20_BL), na.rm = TRUE) - 1) / 4,
    neg_aff = mean(c(anger, stress, sad, anx, guil, tire, lone), na.rm = TRUE)
  ) |>
  ungroup() |>
...
saveRDS(data_pp, "Data/data_pp_goals.RDS")
saveRDS(data, "Data/data_goals.RDS")
